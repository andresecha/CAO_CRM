...
| `check-watermark.sh` | Verifies every hand- or agent-authored source file carries the copyright/attribution header shown at the top of this file; `--fix` inserts it where missing. Exists because the header has gone missing silently more than once (files copied in from an external working directory, or generated by a tool that doesn't know to preserve it). | `make validate` (`watermark` step), CI |
